...
if [ "$PHPCS" == 1 ]; then
  ...
  ARGS="-p --extensions=php --standard=CakePHPOefenweb --ignore=vendor/ .";
...
if [ "$CODECOVERAGE" == 1 ]; then
  ./Console/cake test $PLUGIN_NAME All$PLUGIN_NAME --stderr --coverage-clover Plugin/$PLUGIN_NAME/clover.xml
  EXIT_CODE="$?"
